...
theme_path = ".config/awesome/themes/default/theme.lua"
...
batwidget  = nil
battery    = "BAT0"
local battery_input = io.open("/sys/class/power_supply/" .. battery .. "/present", "r")

if battery_input ~= nil then
   batwidget = widget({ type = "textbox" })
   -- Register widget
   vicious.register(batwidget, vicious.widgets.bat, "$1 $2% $3", 60, battery)
   batdivider = widget ({type = "textbox" })
   batdivider.text = " | "
end
...
globalkeys = awful.util.table.join(
    -- Screenshot
    awful.key({ }, "Print", function () awful.util.spawn("scrot -e '~/bin/upload $f' /tmp/shoot.png 2>/dev/null") end),
    ...
    awful.key({modkey, "Shift"}, "d", shifty.shift_prev),
    awful.key({modkey, "Shift"}, "n", shifty.shift_next),
    ...
    awful.key({modkey, "Shift"}, "e", shifty.del), -- delete a tag
    ...
    awful.key({modkey,}, "p", shifty.rename), -- rename a tag
...
clientkeys = awful.util.table.join(
    ...
    awful.key({modkey}, "y", awful.client.togglemarked),
    awful.key({modkey,}, "m",
        function(c)
